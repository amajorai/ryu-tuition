...
use crate::{
    ...
    models::{
        resolve_tz, Attempt, GradedBy, Item, ItemKind, NewAttempt, PlannedItem,
        SessionStatus, Skill, TuitionSettings,
    },
    ...
    match outcome {
        grade::Outcome::Decided {
            ...
            if !update.informative {
                result.note = Some(
                    "this answer was recorded but did not move your mastery estimate".into(),
                );
            }
        }
...
async fn mark_against_rubric(
    ...
    let raw = host
        ...
        .await?;
    let parsed: serde_json::Value = extract_json(&raw)
        .ok_or_else(|| anyhow!("the grading model did not return JSON"))?;
    let score = parsed
        .get("score")
...
pub async fn generate_items(
    ...
    let raw = host
        ...
        .await?;
    let parsed = extract_json(&raw).ok_or_else(|| anyhow!("the generation model did not return JSON"))?;
    let proposed = parsed
        .get("items")
...
pub async fn mastery_report(state: &AppState, subject_id: &str, now: i64) -> Result<MasteryReport> {
    ...
    let entries: Vec<SkillMastery> = skills
        ...
        .map(|skill| {
            ...
            let projection = bkt::correct_answers_to_target(skill.mastery, skill.params, guess, target);
            SkillMastery {
                skill_id: skill.id.clone(),
