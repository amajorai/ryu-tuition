...
pub enum AnswerKey {
    ...
    /// The stored JSON did not decode.
    ///
    /// Present so one corrupt row cannot fail a whole list query — but it is a
    /// dead end on purpose: [`AnswerKey::encode`] refuses to write it back, so a
    /// read-modify-write through this variant cannot overwrite a recoverable
    /// answer with the shell of it, and the grader refuses to grade it rather than
    /// inventing a verdict.
    Malformed {
        raw: String,
    },
