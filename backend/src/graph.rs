    ...
    skills
        ...
        .filter(|skill| {
            graph
                ...
                .unwrap_or(&[])
                .iter()
                .all(|prereq| mastery.get(prereq).is_some_and(|m| *m >= ready_threshold))
        })
        .collect()
}
...
mod tests {
    ...
    fn a_cycle_is_named_with_the_skills_a_person_recognizes() {
        let cycle = find_cycle(&edges(&[("skl_a", "skl_b"), ("skl_b", "skl_a")])).expect("cyclic");
        let names: BTreeMap<&str, &str> = [("skl_a", "Limits"), ("skl_b", "Continuity")]
            .into_iter()
            .collect();
        assert_eq!(
