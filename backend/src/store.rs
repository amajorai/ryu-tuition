...
//! Outside a session (the `tuition.log` MCP tool) the bind step is simply absent.
//!
...
const COLS_SOURCE: &str = "id, subject_id, kind, title, uri, parsed_text, parser, created_at";
...
impl TuitionStore {
    ...
    pub async fn delete_skill(&self, id: &str) -> Result<bool> {
        ...
        tx.execute("DELETE FROM session_items WHERE skill_id = ?1", params![id])?;
...
fn prereq_path_to(conn: &Connection, from: &str, target: &str) -> Result<Option<Vec<String>>> {
    ...
    while let Some(current) = frontier.pop() {
        ...
        if expansions > MAX_PREREQ_WALK {
            anyhow::bail!("the prerequisite graph is too large or already cyclic to check safely");
        }
...
impl TuitionStore {
    ...
    pub async fn list_attempts_for_skill(
        &self,
        skill_id: &str,
        limit: i64,
    ) -> Result<Vec<Attempt>> {
...
mod tests {
    ...
    async fn a_session_plan_is_ordered_and_each_slot_is_answered_once() {
        ...
        assert!(!s
            ...
            .unwrap());
        assert_eq!(
            s.next_session_item(&session.id)
                .await
                .unwrap()
                .unwrap()
                .item_id,
    ...
    async fn a_redelivered_candidate_payload_is_a_no_op() {
        ...
        assert!(s
            .enqueue_candidates("candidate:x:y:z", &future)
            .await
            .is_err());
    }
    ...
    async fn candidates_for_an_unknown_subject_are_refused() {
        ...
        assert!(s
            .enqueue_candidates("candidate:k:3", &orphan)
            .await
            .is_err());
    ...
    async fn a_candidate_can_only_be_decided_once() {
        ...
        s.enqueue_candidates("candidate:k:1", &envelope)
            .await
            .unwrap();
    ...
    async fn due_skills_include_new_work_and_overdue_reviews_but_never_proposed_ones() {
        ...
        assert!(s
            ...
            .unwrap());
        let due = s
            .list_due_skills(Some(&subject.id), 2_000, 0)
            .await
            .unwrap();
        assert_eq!(due.len(), 1);
        ...
        assert!(
            !early
                .iter()
                .any(|k| k.id == skill.id && k.due_at == Some(1_000)),
    ...
    async fn an_event_claim_holds_for_its_cooldown_and_can_be_cleared() {
        ...
        assert!(s
            .claim_event(at_risk, &subject.id, "", 0, day)
            .await
            .unwrap());
        assert!(!s
    ...
    async fn a_brand_new_skill_is_due_immediately() {
        ...
        let fresh = store
            ...
            .expect("a skill");
        assert!(
            fresh.due_at.is_none(),
            "a new skill starts with no due date"
        );
