...
mod tests {
    ...
    fn the_posterior_update_matches_hand_computed_bayes() {
        ...
        for case in cases {
            ...
            assert!(got.after.is_finite(), "{}: produced a non-finite posterior", case.name);
        }
    }
    ...
    fn the_guess_rate_floor_is_per_item_kind() {
        ...
        let cases: &[(&str, ItemKind, usize, f64)] = &[
            ...
            ("five-choice mcq is exactly the stored rate", ItemKind::Mcq, 5, 0.20),
            ("ten-choice mcq keeps the stored rate", ItemKind::Mcq, 10, 0.20),
            ...
            ("one-choice mcq is not floored to certainty", ItemKind::Mcq, 1, 0.20),
            ("zero-choice mcq is not floored either", ItemKind::Mcq, 0, 0.20),
            ("cloze has no structural floor", ItemKind::Cloze, 4, 0.20),
            ("numeric has no structural floor", ItemKind::Numeric, 0, 0.20),
            ("free response has no structural floor", ItemKind::Free, 0, 0.20),
        ];
