...
async fn raise_review_due(state: &AppState, now: i64) {
    ...
    for subject in subjects {
        let Ok(all_due) = state
            .store
            .list_due_skills(Some(&subject.id), now, 100)
            .await
        else {
            continue;
        };
        ...
        state
            ...
            .emit_with_notify(
                ...
                // A scheduled-review reminder is the ideal notification: time-bound,
                // once a day, and worth being pushed at.
                Some(
                    ryu_app_events::NotifyHint::info(
                        format!("{} review due", subject.name),
                        Some(format!("{} cards ready to review", due.len())),
                    )
                    .with_level("warning"),
                ),
            )
        ...
        for skill in skills {
            ...
            state
                ...
                .emit_with_notify(
                    ...
                    // A skill regressing below ready is worth a nudge back to practice.
                    Some(
                        ryu_app_events::NotifyHint::info(
                            format!("“{}” needs review", skill.name),
                            Some(format!("Mastery dropped below {}%", skill.mastery)),
                        )
                        .with_level("warning"),
                    ),
                )
    ...
    for subject in subjects {
        ...
        state
            ...
            .emit_with_notify(
                ...
                // An exam at risk is the most time-sensitive reminder this app has.
                Some(
                    ryu_app_events::NotifyHint::info(
                        format!("{} exam at risk", subject.name),
                        Some(format!(
                            "{} days left — {} sessions needed",
                            projection.days_remaining,
                            finite(projection.sessions_needed)
