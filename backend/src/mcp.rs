...
//! - **Tool names are registered BARE.** Core forms the id as `{server}.{tool}` from
//!   the `mcp_servers` key in the manifest, so registering `tuition.due` here would
//!   produce `tuition.tuition.due`. The manifest's `provides[].tools` map carries the
...
mod tests {
    ...
    fn no_tool_name_is_self_prefixed() {
        // Core forms `{server}.{tool}`, so a name carrying the server prefix here
        // becomes `tuition.tuition.due` and no caller can ever reach it.
